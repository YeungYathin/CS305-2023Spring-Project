...
class Config():
    ...
    lease_time = 10000000  # 14 * 24 * 60 * 60
...
class DHCPServer():
    ...
    def assemble_ack(cls, pkt, datapath, port):
        ...
        if (ip_addr not in cls.offer_pool):
            nak_pkt = cls.assemble_nak(pkt, datapath, port)
            return nak_pkt
        else:
            cls.offer_pool.remove(ip_addr)
        ...
        dhcp_options.option_list.append(
            dhcp.option(tag=dhcp.DHCP_SERVER_IDENTIFIER_OPT, value=addrconv.ipv4.text_to_bin(cls.ip_server)))
        dhcp_options.option_list.append(
    ...
    ###################################################################################################################
    @classmethod
    def assemble_nak(cls, pkt, datapath, port):

        # TODO: Generate DHCP NAK packet here
        eth_pkt = pkt.get_protocol(ethernet.ethernet)
        if eth_pkt.ethertype != 0x0800:
            return
        ip_pkt = pkt.get_protocol(ipv4.ipv4)
        ...
        nak_pkt.add_protocol(ethernet.ethernet(
            ...
            dst=eth_pkt.src,
            src=cls.hardware_addr))
        nak_pkt.add_protocol(ipv4.ipv4(
            dst='255.255.255.255',
            src=cls.ip_server,
            proto=ip_pkt.proto))
        nak_pkt.add_protocol(udp.udp(
            src_port=67,
            dst_port=68))
        dhcp_options = dhcp.options()
        dhcp_options.option_list.append(dhcp.option(tag=dhcp.DHCP_MESSAGE_TYPE_OPT, value=b'\x06'))
        dhcp_options.option_list.append(
            dhcp.option(tag=dhcp.DHCP_SERVER_IDENTIFIER_OPT, value=addrconv.ipv4.text_to_bin(cls.ip_server)))
        nak_pkt.add_protocol(dhcp.dhcp(
            op=dhcp.DHCP_BOOT_REPLY,
            chaddr=dhcp_pkt.chaddr,
            flags=dhcp_pkt.flags,
            giaddr=dhcp_pkt.giaddr,
